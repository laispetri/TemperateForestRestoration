...
FinalModel<-"model{
 ...
 for(i in 1:P){ #number of subplots
    ...
    C[i,1]<-B[treatment[i],1]*Cover2019[i]+alpha[1]*RichN[i,2]+alpha[2]*SLA[i,2]+alpha[3]*light[i,2]+alpha[4]*sm[i,2] #process model for 2020
    ...
    C[i,3]<-B[treatment[i],3]*(w[1]*grassAG21[i]+w[2]*forbsAG21[i]+w[3]*woodyAG21[i])+alpha[1]*RichN[i,2]+alpha[2]*SLA[i,2]+alpha[3]*light[i,2]+alpha[4]*sm[i,2] #process model for 2022
...
write_csv(Stats, file.path(dataout,"ParametersStatistics.csv"))
Quantiles <- as.data.frame(parameters0[2]) %>% mutate_if(is.numeric, round,digits=4) %>% rownames_to_column(., var = "ParameterID")
write_csv(Quantiles, file.path(dataout,"ParametersQuantiles.csv"))
...
ggsave(file.path(graph.wd,"ModelFit.png"), plot = last_plot(), width=7, height=5, dpi = 300, units="in")
...
DataPlot %>% 
  ...
  annotate("text", x=1.3,y=0.77,label="ac",size=5) +
  annotate("text", x=1.6,y=0.54,label="a",size=5) +
  annotate("text", x=1.9,y=0.75,label="ac",size=5) +
  annotate("text", x=2.2,y=0.55,label="a",size=5) +
  annotate("text", x=2.5,y=0.45,label="a",size=5) +
  annotate("text", x=3,y=1.55,label="be",size=5) +
  annotate("text", x=3.3,y=2.13,label="be",size=5) +
  annotate("text", x=3.6,y=1.3,label="bce",size=5) +
  annotate("text", x=3.9,y=2.1,label="be",size=5) +
  annotate("text", x=4.2,y=1.15,label="bcd",size=5) +
  annotate("text", x=4.5,y=2.1,label="be",size=5) +
  annotate("text", x=5,y=1.02,label="bcd",size=5) +
  annotate("text", x=5.3,y=1.35,label="bcd",size=5) +
  annotate("text", x=5.6,y=0.83,label="bcd",size=5) +
  annotate("text", x=5.9,y=1.35,label="bcd",size=5) +
  annotate("text", x=6.2,y=0.73,label="bcd",size=5) +
  annotate("text", x=6.5,y=1.25,label="bcd",size=5)

ggsave(file.path(graph.wd,"Q1.png"), plot = last_plot(), width=8, height=5, dpi = 300, units="in")
```
...
ggsave(file.path(graph.wd,"GrowthForms.png"), plot = last_plot(), width=6, height=5, dpi = 300, units="in")
...
ALPHAmodelpar %>% 
  ...
  scale_x_continuous(breaks=c(1,1.95,2.9,4),
                     ...
                     limits=c(0.6,4.25))+
  ylab("Association with native cover\n(mean+95%CI)")+
  theme_bw()+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
ggsave(file.path(graph.wd,"CovariatesEffects.png"), plot = last_plot(), width=6.8, height=5.2, dpi = 300, units="in")
```
...
ggsave(file.path(graph.wd,"CumVPD_oneSite.png"), plot = last_plot(), width=6.8, height=5.2, dpi = 300, units="in")
```
...
ggsave(file.path(graph.wd,"TraitsSeededSppv2.png"), plot = last_plot(), width=6.8, height=5.2, dpi = 300, units="in")
...
ggsave(file.path(graph.wd,"TraitsSeededSppFreq.png"), plot = last_plot(), width=6.8, height=5.2, dpi = 300, units="in")
```
...
ggsave(file.path(graph.wd,"ResidualsIAS.png"), plot = last_plot(), width=7, height=6, dpi = 300, units="in")
```
...
ggsave(file.path(graph.wd,"ESCoverVsRich.png"), plot = last_plot(), width=6.8, height=8, dpi = 300, units="in")
...
ggsave(file.path(graph.wd,"NatCoverVsYear.png"), plot = last_plot(), width=8.5, height=10, dpi = 300, units="in")
```
