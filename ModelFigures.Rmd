...
Which species from the seeding mixture were found in any of the forest understories?
```{r}
PE_divSpp <- read_csv("extraDataRevisions/PE_divSpp.csv") %>% 
  filter(subplot %in% c(1,7)) # only control plots 
PE_divSpp %>% 
  filter(acronym %in% c("ACHMIL", "AGEALT", "AQUCAN", "ASCTUB", "DESCAD", "ELYCAN", "ELYHYS",
                        "MONFIS", "PENDIG", "RUDHIR", "RUDTRI", "SYMOOL", "THADAS", "THADIO")) %>% 
  distinct(sppName,acronym) %>% 
  arrange(acronym)

# species not found in the control plots: "DESCAD", "SYMOOL", "THADAS"
```
...
ggsave(file.path(graph.wd,"RawDiffNatCov.png"), plot = last_plot(), width=6.8, height=5.2, dpi = 300, units="in")
